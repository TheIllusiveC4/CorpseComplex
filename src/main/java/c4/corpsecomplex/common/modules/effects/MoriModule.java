...
public class MoriModule extends Submodule {
  ...
  public void onPlayerRespawnFinish(
    ...
    if (!e.player.world.isRemote && !e.isEndConquered()) {
      ...
      PotionEffect moriEffect = new PotionEffect(moriPotion, duration * 20, 0,
              false, true);
  ...
  public void onEatingFood(PlayerInteractEvent.RightClickItem e) {
    ...
    if ((e.getEntityPlayer().isPotionActive(MoriModule.moriPotion)) &&
            (e.getItemStack().getItemUseAction() == EnumAction.EAT)) {
      e.setCanceled(true);
    }
  }
  ...
  public void onEatingCake(PlayerInteractEvent.RightClickBlock e) {
    ...
    if (!noFood || !e.getEntityPlayer().isPotionActive(MoriModule.moriPotion)) {
      return;
    }

    if (e.getWorld().getBlockState(e.getPos())
            .getBlock() instanceof BlockCake) {
      e.setCanceled(true);
    }
  }
  ...
  public MoriModule(Module parentModule) {
    ...
    configCategory.setComment(
            "Customize your own respawn effect for modifying player " +
                    "attributes");
  }

  public void loadModuleConfig() {
    setCategoryComment();
    cfgEnabled = getBool("Enable Custom Respawn Effect", false,
            "Set to true to enable custom effect applied on respawn", true);
    modHealth = getDouble("Maximum Health Modifier", 0, -1024, 1024,
            "Set maximum health modifier", false);
    modArmor = getDouble("Armor Modifier", 0, -30, 30, "Set armor modifier",
            false);
    modToughness = getDouble("Armor Toughness Modifier", 0, -20, 20,
            "Set armor toughness modifier", false);
    modMove = getDouble("Movement Speed Percent Modifier", 0.0f, -1.0f, 1.0f,
            "Set movement speed percent modifier", false);
    modDamage = getDouble("Attack Damage Modifier", 0.0f, -2048.0f, 2048.0f,
            "Set attack damage modifier", false);
    modSpeed = getDouble("Attack Speed Percent Modifier", 0.0f, -1.0f, 1.0f,
            "Set attack speed percent modifier", false);
    doRecover = getBool("Gradual Recovery", false,
            "Set to true to enable gradual recovery (modifiers will diminish " +
                    "gradually as the effect goes on)",
            false);
    duration = getInt("Duration", 0, 0, 1600,
            "Set duration (seconds) for the effect", false);
    noFood = getBool("Cannot Eat Food", false,
            "Set to true to disable eating food while effect is active", false);
    noXP = getBool("Cannot Gain XP", false,
            "Set to true to disable gaining experience while effect is active",
            false);
    curativeItems = getStringList("Curative Items",
            new String[] {"minecraft:milk_bucket"},
            "List of items that can cure the effect", false);
  ...
  public void initPropOrder() {
    propOrder = new ArrayList<>(
            Arrays.asList("Enable Custom Respawn Effect", "Duration",
                    "Curative Items", "Gradual Recovery",
                    "Maximum Health Modifier", "Armor Modifier",
                    "Armor Toughness Modifier", "Attack Damage Modifier",
                    "Attack Speed Percent Modifier",
                    "Movement Speed Percent Modifier"));
  }
