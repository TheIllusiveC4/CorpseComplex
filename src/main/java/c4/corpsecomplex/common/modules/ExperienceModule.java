...
public class ExperienceModule extends Module {
  ...
  public void loadModuleConfig() {
    setCategoryComment();
    cfgEnabled = getBool("Enable Experience Module", false,
            "Set to true to enable experience module", false);
    keepXP = getBool("Keep All XP", false,
            "Set to true to keep all XP on death", false);
    xpLoss = getDouble("Lost XP Percent", 1, 0, 1,
            "Percent of experience lost on death", false);
    xpRecover = getDouble("Recoverable XP Percent", 0.2F, 0, 1,
            "Percent of lost experience that can be recovered", false);
    maxXPRecover = getInt("Maximum Recoverable XP", 0, 0, 10000,
  ...
  public void initPropOrder() {
    propOrder = new ArrayList<>(
            Collections.singletonList("Enable Experience Module"));
  }
  ...
  private static void setExperiencesValues(LivingExperienceDropEvent e) {
    ...
    if (keepXP) {
      ...
      int dropXP = (int) Math.round(
              player.experienceTotal * xpLoss * xpRecover);
