...
public class ItemScroll extends Item {
    ...
    {
        if (!worldIn.isRemote) {
            IDeathLocation deathLoc = playerIn.getCapability(DeathLocation.Provider.DEATH_LOC_CAP, null);
            if (deathLoc == null || deathLoc.hasUsedScroll()) {
                playerIn.sendStatusMessage(new TextComponentTranslation("item.scroll.noUse"), true);
                return new ActionResult<>(EnumActionResult.FAIL, playerIn.getHeldItem(handIn));
            } else if (deathLoc.getDeathLocation() == null) {
                playerIn.sendStatusMessage(new TextComponentTranslation("item.scroll.noLocation"), true);
                return new ActionResult<>(EnumActionResult.FAIL, playerIn.getHeldItem(handIn));
            } else {
    ...
    public ItemStack onItemUseFinish(ItemStack stack, World world, EntityLivingBase entity) {
        ...
        if (entity instanceof EntityPlayer && !world.isRemote) {
            ...
            if (deathLoc != null) {
                ...
                ScrollTeleporter.teleportToPosition(player, pos, player.getHorizontalFacing(), deathLoc.getDeathDimension());
