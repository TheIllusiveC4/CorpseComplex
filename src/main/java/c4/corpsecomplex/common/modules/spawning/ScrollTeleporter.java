...
public class ScrollTeleporter {
  ...
  public static void sendTeleportEffect(World world, BlockPos pos) {
    NetworkHandler.INSTANCE.sendToAllAround(new TeleportEffectMessage(pos),
   ...
   * MIT License
   * Waystone's WaystoneManager (https://github.com/blay09/Waystones/blob/1
   * .12/src/main/java/net/blay09/mods/waystones/WaystoneManager.java)
   * =======================================================================================================================================
   */
  public static void teleportToPosition(EntityPlayer player, BlockPos pos,
          EnumFacing facing, int dimensionId) {
    sendTeleportEffect(player.world, new BlockPos(player));
    if (dimensionId != player.getEntityWorld().provider.getDimension()) {
      MinecraftServer server = player.world.getMinecraftServer();
      if (server != null) {
        transferPlayerToDimension((EntityPlayerMP) player, dimensionId,
    ...
    if (player instanceof EntityPlayerMP) {
      NetworkHandler.INSTANCE.sendTo(new TeleportSoundMessage(),
   ...
   * CofHCore's EntityHelper (https://github.com/CoFH/CoFHCore/blob/1
   * .12/src/main/java/cofh/core/util/helpers/EntityHelper.java)
   * =======================================================================================================================================
   */
  private static void transferPlayerToDimension(EntityPlayerMP player,
          int dimension, PlayerList manager) {

    int oldDim = player.dimension;
    WorldServer worldserver = manager.getServerInstance().getWorld(
            player.dimension);
    player.dimension = dimension;
    WorldServer worldserver1 = manager.getServerInstance().getWorld(
            player.dimension);
    ...
    for (PotionEffect potioneffect : player.getActivePotionEffects()) {
      player.connection.sendPacket(
              new SPacketEntityEffect(player.getEntityId(), potioneffect));
    }
    FMLCommonHandler.instance().firePlayerChangedDimensionEvent(player, oldDim,
            dimension);
  }

  private static void transferEntityToWorld(Entity entity, WorldServer oldWorld,
          WorldServer newWorld) {
    WorldProvider oldWorldProvider = oldWorld.provider;
    WorldProvider newWorldProvider = newWorld.provider;
    double moveFactor = oldWorldProvider.getMovementFactor() /
            newWorldProvider.getMovementFactor();
    ...
    if (entity.isEntityAlive()) {
      entity.setLocationAndAngles(x, entity.posY, z, entity.rotationYaw,
