...
public class TANModule extends Module {
  ...
  public void onPlayerRespawnBegin(PlayerEvent.Clone e) {
    ...
    if (!e.isWasDeath() || e.getEntityPlayer().getEntityWorld().isRemote) {
      return;
    }
  ...
  public void onPlayerRespawnFinish(
          net.minecraftforge.fml.common.gameevent.PlayerEvent.PlayerRespawnEvent evt) {
  ...
  public TANModule() {
    super("Tough as Nails",
            "Customize keeping Tough as Nails attributes on respawn");
  }

  public void loadModuleConfig() {
    setCategoryComment();
    cfgEnabled = getBool("Enable TAN Module", false,
            "Set to true to enable Tough as Nails module", false);
    keepThirst = getBool("Keep Thirst Level", false,
            "Set to true to retain thirst level on respawn", false);
    keepHydration = getBool("Keep Hydration Level", false,
            "Set to true to retain hydration level on respawn", false);
    keepTemperature = getBool("Keep Temperature Level", false,
            "Set to true to retain temperature level on respawn", false);
    minThirst = getInt("Minimum Thirst Level", 6, 0, 20,
            "Lowest amount of thirst you can respawn with", false);
    maxThirst = getInt("Maximum Thirst Level", 20, minThirst, 20,
            "Maximum amount of thirst you can respawn with", false);
  }
  ...
  @Optional.Method(modid = MOD_ID)
  private static void restoreStats(EntityPlayer player,
          EntityPlayer oldPlayer) {

    int thirst = ThirstHelper.getThirstData(oldPlayer).getThirst();
    int temp = TemperatureHelper.getTemperatureData(oldPlayer).getTemperature()
            .getRawValue();
  ...
  private static void updateClientTAN(EntityPlayer player) {
    ...
    int temp = TemperatureHelper.getTemperatureData(player).getTemperature()
            .getRawValue();

    NetworkHandler.INSTANCE.sendTo(new TANMessage(thirst, temp),
            (EntityPlayerMP) player);
  }

  @Optional.Method(modid = MOD_ID)
  private static void restoreThirst(EntityPlayer player, int thirst) {
    ThirstHelper.getThirstData(player).setThirst(
            Math.max(minThirst, (Math.min(maxThirst, thirst))));
  }
  ...
  private static void restoreTemperature(EntityPlayer player, int temp) {
    TemperatureHelper.getTemperatureData(player).setTemperature(
