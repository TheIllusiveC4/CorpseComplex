...
public class TeleportEffectMessage implements IMessage {
  ...
  public static class TeleportEffectMessageHandler
          implements IMessageHandler<TeleportEffectMessage, IMessage> {

    @Override
    @SideOnly(Side.CLIENT)
    public IMessage onMessage(TeleportEffectMessage message,
            MessageContext ctx) {
      ...
      mainThread.addScheduledTask(() -> {
        ...
        for (int x = 0; x < 128; x++) {
          ...
          double d3 = message.pos.getX() +
                  (rand.nextDouble() - 0.5D) * playerSP.width * 2.0D;
          double d4 = message.pos.getY() + rand.nextDouble() * playerSP.height;
          double d5 = message.pos.getZ() +
                  (rand.nextDouble() - 0.5D) * playerSP.width * 2.0D;
          playerSP.world.spawnParticle(EnumParticleTypes.PORTAL, d3, d4, d5,
